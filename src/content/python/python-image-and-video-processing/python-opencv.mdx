...
imgOutput = cv2.warpPerspective(img, matrix, (image_width, image_height)) # top-left, top-right, bottom-left, bottom-right
...
def pick_color(mouse_event, x, y):
    global lower_color, upper_color, current_frame
    
    if mouse_event == cv2.EVENT_LBUTTONDOWN and current_frame is not None:
...
for question, idx in enumerate(range(0, len(question_contours), 4)):
    ...
    colour = (0, 255, 0) if correct_answer == marked[1] else (0, 0, 255)
...
colour = (50, 168, 82) if score >= 0.6 else (71, 7, 219)
