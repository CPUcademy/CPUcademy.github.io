...
def count_letters(textS):
    ...
    for char in textS.upper():
        if char in ALPHABET:
            counts[char] += 1 # counting only A-Z characters
    return counts

def sort_by_frequency(textS):
    freq_map = count_letters(textS)
...
def match_english_profile(textS):
    freq_order = sort_by_frequency(textS) # getting ranked letters from ciphertext segment
...
def find_repeated_sequences(textS):
    cleaned = NON_LETTER_REGEX.sub("", textS.upper()) # stripping non‑letters so repeated patterns align cleanly
    spacings = {}
...
def extract_nth_letters(n, key_len, textS):
    cleaned = NON_LETTER_REGEX.sub("", textS.upper()) # removing punctuation so indexing aligns with key positions
