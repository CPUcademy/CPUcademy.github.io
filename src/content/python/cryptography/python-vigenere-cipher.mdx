...
def find_repeated_sequences(text):
    ...
    spacings = {}
    for sequence_length in range(3, 6): # typical repeated substring lengths for Kasiski
        for i in range(len(cleaned) - sequence_length):
            seq = cleaned[i:i + sequence_length] # candidate repeated sequence
            for j in range(i + sequence_length, len(cleaned) - sequence_length):
                if cleaned[j:j + sequence_length] == seq:
