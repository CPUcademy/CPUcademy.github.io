...
class ECommercePlatform:
    ...
    def apply_discounts(self, product_query):
        self.discount_handlers(product_query) # calling all registered discount handlers

class Product:
    def __init__(self, product_platform, name, base_price):
    ...
    @property
    def price(self):
        product_query = Query(self.name, "price", self.base_price)
        self.product_platform.apply_discounts(product_query) # applying registered discounts
        return product_query.value

class DiscountModifier:
    def __init__(self, product_platform, product):
        self.product_platform = product_platform
        self.product = product
        self.product_platform.discount_handlers.append(self.handle) # registering handler

    def handle(self, product_query):
        pass # to be implemented by subclasses

class BlackFridayDiscount(DiscountModifier):
    def handle(self, product_query):
        if product_query.product_name == self.product.name and product_query.query_type == "price":
            product_query.value *= 0.8 # 20% discount

class LoyaltyDiscount(DiscountModifier):
    def handle(self, product_query):
        if product_query.product_name == self.product.name and product_query.query_type == "price":
            product_query.value *= 0.9 # additional 10% discount for loyal customers

product_platform = ECommercePlatform()
laptop = Product(product_platform, "Laptop", 1000)

print("Original price:", laptop.price)

black_friday = BlackFridayDiscount(product_platform, laptop)
print("After Black Friday discount:", laptop.price)

loyalty = LoyaltyDiscount(product_platform, laptop)
...
class TransactionHistory: # an invoker class (the object that calls the command)
    ...
    def undo_operation(self):
        if self.history:
            ...
            last_command.undo_operation()

source_account = BankAccount(500)
target_account = BankAccount(200)
history = TransactionHistory()

print(f"Source account before transfer: {source_account}")
print(f"Target account before transfer: {target_account}")

transfer1 = TransferMoneyCommand(sousource_accountrce, target_account, 100)
history.execute_command(transfer1)

print(f"Source account after first transfer: {source_account}")
print(f"Target account after first transfer: {target_account}")

transfer2 = TransferMoneyCommand(source_account, target_account, 1000) # trying to transfer more than available funds
history.execute_command(transfer2)

print(f"Source account after second transfer attempt: {source_account}")
print(f"Target account after second transfer attempt: {target_account}")
history.undo_operation()

print(f"Source account after undo: {source_account}")
print(f"Target account after undo: {target_account}")
```
...
class Token:
    ...
    def __repr__(self):
        return f"Token({self.typee}, {self.value})"
...
class Lexer:
    ...
    def get_next_token(self):
        if self.indexx >= len(self.text):
            return None

        char = self.text[self.indexx]
        if char.isdigit():
            num = 0
            # Extracting multi-digit numbers
            while self.indexx < len(self.text) and self.text[self.indexx].isdigit():
                num = num * 10 + int(self.text[self.indexx])
                self.indexx += 1
    ...
    def tokenize(self):
        tokens = []
        while self.indexx < len(self.text):
