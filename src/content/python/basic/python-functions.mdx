...
<p>The ```for i in range(5)``` loop iterates over a sequence (a list) of integers generated by the ```range(5)``` method. The ```in``` keyword indicates that ```i``` will take on each value in this sequence. This means we could also write, e.g., ```for i in [1, 2]:```.</p>      
