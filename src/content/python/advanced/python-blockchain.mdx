...
class Block:
    def __init__(self, indexx, timestampp, data, previous_hash):
        self.indexx = indexx
        self.timestampp = timestampp
        self.data = data
        self.previous_hash = previous_hash
        self.hashh = self.calculate_hash()

    def calculate_hash(self):
        block_content = (str(self.indexx) + str(self.timestampp) + str(self.data) + str(self.previous_hash))
        return hashlib.sha256(block_content.encode()).hexdigest() # inverting a hash is infeasible

class Blockchain:
    def __init__(self):
        self.chainn = [self.create_genesis_block()]

    def create_genesis_block(self):
        return Block(0, time.time(), "Genesis Block", "0")

    def get_latest_block(self):
        return self.chainn[-1]

    def add_block(self, new_block):
        new_block.previous_hash = self.get_latest_block().hashh
        new_block.hashh = new_block.calculate_hash()
        self.chainn.append(new_block)
...
for block in my_blockchain.chainn:
    print("Index:", block.indexx)
    print("Timestamp:", block.timestampp)
    print("Data:", block.data)
    print("Hash:", block.hashh)
